...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		//создание кнопки
		hButton_V_P = CreateWindow(L"button", L"Complete the process", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON, 300, 470, 250, 40, hwnd, (HMENU)ID_BUTTON_V_P, hInstance, NULL);
	...
	case WM_COMMAND:
		...
		case ID_BUTTON_Z_P://задание приоритета процесса
			...
			if (h == INVALID_HANDLE_VALUE) {
				MessageBox(hwnd, TEXT("Error:INVALID_HANDLE_VALUE!"),
					TEXT("Error"), MB_OK | MB_ICONERROR);
				return FALSE;
			}

			if (!SetPriorityClass(h, Prior))
				MessageBox(hwnd, TEXT("Priority cannot be set!"),
					TEXT("Error"), MB_OK | MB_ICONERROR);
			else
		...
		case ID_BUTTON_LOG_OFF:
			if (MessageBox(hwnd, TEXT("Are you sure to log out?"),
				TEXT("Attention"), MB_YESNO | MB_ICONWARNING) == IDYES)
			{
				if (!SetCurrentPrivilege(SE_SHUTDOWN_NAME, TRUE))
				{
		...
		case ID_BUTTON_REBOOT:
			if (MessageBox(hwnd, TEXT("Are you sure you want to restart your computer?"),
				TEXT("Attention"), MB_YESNO | MB_ICONWARNING) == IDYES)
			{
				if (!SetCurrentPrivilege(SE_SHUTDOWN_NAME, TRUE))
				{
...
int ProcessesInfo(HWND hwnd, DWORD dwProcessID)
{
	...
	if (Process32First(myhandle, &pe32))
		...
		wsprintf(sz, TEXT("Min/max physical memory size:%lu / %lu")
			, minphmem, maxphmem);
